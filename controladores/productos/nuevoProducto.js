...
async function guardarProducto(){
    ...
    if(Number(precioCosto)>0 && Number(pvp)>0){
        await $.ajax({
            ...
            data: { crearProducto: true, nombre: nombre, descripcion: descripcion, cantidad: cantidad, precioCosto: precioCosto, pvp: pvp, imagen: imagen },
            type: "POST",
            success: function (data) {
                if(data == "exito"){
                    mensajeExito("Productos", "El producto se ha guardado con éxito")
                    $('#limpiarProducto').click()
